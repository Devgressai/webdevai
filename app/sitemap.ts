 ...
 * 
 * This is the primary sitemap file that Next.js recognizes for metadata routes.
 * It generates the sitemap index that points to all child sitemaps.
 * 
 * ARCHITECTURE:
 * - Next.js calls generateSitemaps() to get list of sitemap IDs
 * - Next.js calls default export for each ID to get entries
 * - Each sitemap can contain up to 50,000 URLs
 * - Runs at BUILD TIME, not request time - this ensures file system access works
 */
 ...
 * Next.js calls this at BUILD TIME to know how many child sitemaps to generate.
 */
export async function generateSitemaps(): Promise<Array<{ id: number }>> {
  ...
  console.log(`  - /sitemap/2.xml (blog): ${blogEntries.length} URLs`)
 ...
 * Next.js calls this at BUILD TIME for each ID returned by generateSitemaps().
 * 
