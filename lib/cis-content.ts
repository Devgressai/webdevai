...
export async function fetchCisDoc(citySlug: string, industrySlug: string, service: string): Promise<CisDoc | null> {
  const query = `*[_type == "cisPage" && service == $service && references(*[_type=="city" && slug.current==$city]._id) && references(*[_type=="industry" && slug.current==$industry]._id)] | order(_updatedAt desc)[0]{
